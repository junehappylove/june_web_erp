...
<body>
	<a href="javascript:void(0);" onclick="upload();return false;">点击这里直接返回文件URL</a>
	|
	<a href="javascript:void(0);" onclick="unloadme();return false;">点击这里关闭模式窗口</a>
	<br />
	<p style="color: #999;">
		注：此页面仅供演示用，您可以在此页面的基础上实现文件上传和用户文件浏览功能<br />本页面提供两个接口用来和编辑器进行互动：callback和unloadme，callback用来返回上传的地址，唯一返回参数就是返回值，unloadme方法用来关闭当前模式窗口，无任何参数
	</p>
	<form id="frmUpload" method="post" action="uploadguiupload.php">
		<input type="hidden" name="parenthost"
			value="<?php echo $_GET['parenthost'];?>" /> <input type="submit"
			name="save" value="点击这里提交上传表单" />
	</form>
</body>
</html>