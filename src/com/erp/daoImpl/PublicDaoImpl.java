...
public class PublicDaoImpl<T> implements PublicDao<T> {
	 ...
	 * sessionFactory).getServiceRegistry().getService(EventListenerRegistry.
	 * class); registry.getEventListenerGroup(EventType.POST_COMMIT_INSERT).
	 * appendListener(new PostInsert());
	 * registry.getEventListenerGroup(EventType.POST_COMMIT_UPDATE).
	 * appendListener(new PostUpdate()); }
	 */
	...
	public Serializable save(T o) {
		...
		Constants.getLogs(this.getCurrentSession(), o, Constants.LOGS_INSERT, Constants.LOGS_INSERT_TEXT,
				Constants.LOGS_INSERT_NAME);
		return serializable;
	}
	...
	public void update(T o) {
		...
		Constants.getLogs(this.getCurrentSession(), o, Constants.LOGS_UPDATE, Constants.LOGS_UPDATE_TEXT,
				Constants.LOGS_UPDATE_NAME);
	}

	public void deleteToUpdate(T o) {
		this.getCurrentSession().update(o);
		Constants.getLogs(this.getCurrentSession(), o, Constants.LOGS_DELETE, Constants.LOGS_DELETE_TEXT,
				Constants.LOGS_DELETE_NAME);
	}
