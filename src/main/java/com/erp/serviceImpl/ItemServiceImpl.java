...
public class ItemServiceImpl implements ItemService {
	...
	/*
	 * (非 Javadoc) <p>Title: addBrands</p> <p>Description:增加品牌 </p>
	 * 
	 * @param name
	 * 
	 * @return
	...
	public Item findItemByMyid(String myid, Integer suplierId) {
		...
		if (list != null && list.size() != 0) {
			...
			if (suplierId != null && !"".equals(suplierId)) {
				String sql = "SELECT t.PRICE  from ORDER_PURCHASE_LINE t LEFT JOIN ORDER_PURCHASE op on t.ORDER_PURCHASE_ID=op.ORDER_PURCHASE_ID where op.SUPLIER_ID="
						+ suplierId + "  and   t.MYID='" + myid + "' and   t.LASTMOD=(\n"
						...
						+ " where tt.SUPLIER_ID=" + suplierId + "  and  ot.STATUS='A' and ot.myid='" + myid
						+ "' GROUP BY ot.myid\n" + ") \n" + "GROUP BY t.LASTMOD";
